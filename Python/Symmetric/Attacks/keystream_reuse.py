...
for guessed_byte in range(256): # iterate on all possible byte value 0->255
    for c in ciphertexts:
        if chr(c[0] ^ guessed_byte) in ascii_letters: # we simulate decryption -> if xor with current guess is in ascii_letters -> candidate byte
            counters[guessed_byte] +=1

max_matches = max(counters)
print(max_matches) # in one case after decryption we have 50 as max ascii letters

match_list = [(counters[i],i) for i in range(256)] # generate list with pair coutners[i] and i -> each byte with value of counter. which byte produced most matches?
# print(match_list) 
